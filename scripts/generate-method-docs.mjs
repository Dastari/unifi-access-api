...
const imports = `import type { EndpointDefinition, EndpointInvoker } from './internal/endpoint.js';\nimport type { endpointDefinitions } from './endpoints/index.js';\n\ntype EndpointFor<K extends keyof typeof endpointDefinitions> =\n  typeof endpointDefinitions[K] extends EndpointDefinition<infer P, infer Q, infer B, infer R, infer F>\n    ? EndpointInvoker<EndpointDefinition<P, Q, B, R, F>>\n    : never;\n\n`;
...
for (const [name, def] of entries) {
  ...
  lines.push('     */');
  lines.push(`    readonly ${name}: EndpointFor<'${name}'>;`);
  lines.push('');
}
lines.push('  }');
lines.push('}\n');
